...
namespace Uygunitem.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult urunDetay(int id = 0)
        {
            ...
            {
                var ürünler = db.cekilen_datalar.Where(x => x.cekilen_urun_id == id).OrderBy(x => x.cekilen_tarih);
                
                foreach (var item in ürünler)
                {
                   
                    if(viewModel.tablo.ContainsKey(item.cekilen_firma)&& item.cekilen_urun_id==id)
                    {
                        string gfiyat = item.cekilen_fiyat.ToString();
                        viewModel.tablo[item.cekilen_firma] += gfiyat.Replace(",",".")+",";
                    }
                    else if(item.cekilen_urun_id == id)
                    {
                        string gfiyat = item.cekilen_fiyat.ToString();
        ...
        public ActionResult urunTakip(FormCollection takipform, int id)
        {
            ...
            model.takip_urun = id;
            model.takip_sonfiyat = viewModel.cekilen_datalar.Where(x => x.cekilen_urun_id == id).Where(x => x.cekilen_durum == 1).OrderBy(x => x.cekilen_fiyat).Select(x => x.cekilen_fiyat).FirstOrDefault();
